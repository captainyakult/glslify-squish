...
function minifier(safe_words, mutate_storages) {
  ...
  function mutate(node) {
    ...
    // this stops any property of a struct from being mutated
    // todo: BETTER - check whether the struct is being declared as a uniform
    if (node.parent && is_struct_property(node)) return
  ...
  function is_struct_property(node) {
    return node.parent.parent && node.parent.parent.parent && node.parent.parent.parent.type === 'struct'
  }
